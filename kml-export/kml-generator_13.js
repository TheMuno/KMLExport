function buildKml(tripData) {
  ...
  const arrivalAirportPlacemark = tripData.arrivalAirport ? buildAirportPlacemark(tripData.arrivalAirport, 'Arrival Airport') : '';
  const departureAirportPlacemark = tripData.departureAirport ? buildAirportPlacemark(tripData.departureAirport, 'Departure Airport') : '';
...
<Document>
  ...
  ${styleBlocks}
  ${hotelPlacemark}
  ${arrivalAirportPlacemark}
  ${departureAirportPlacemark}
  ${dayFolders}
</Document>
</kml>`;
}
...
function buildAirportPlacemark(airport, label) {
  if (!airport.lat || !airport.lng) return '';
  return `
  <Placemark>
    <name>${escapeXml(airport.name)} (${label})</name>
    <styleUrl>#airport</styleUrl>
    <Point>
      <coordinates>${airport.lng},${airport.lat},0</coordinates>
    </Point>
  </Placemark>`;
}
